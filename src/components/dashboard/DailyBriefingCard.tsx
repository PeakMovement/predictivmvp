...
function CollapsibleSection({ title, icon, preview, children, variant = "default" }: CollapsibleSectionProps) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div className={cn(
        "rounded-md border border-[#D5D6CE]/50 transition-all duration-200",
        variant === "warning"
          ? "bg-destructive/5"
...
function RecommendationItem({ recommendation, index }: RecommendationItemProps) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      <div className={cn(
        "rounded-md border border-[#D5D6CE]/50 transition-all duration-200 bg-card hover:brightness-105",
