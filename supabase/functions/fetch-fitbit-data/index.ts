...
serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.from("fitbit_data").insert([{ fetched_at: new Date().toISOString(), data }]);

    if (error) throw error;

    return new Response(JSON.stringify({ success: true, message: "Fitbit data synced", data }), {
      status: 200,
      headers: corsHeaders,
    });
  } catch (error) {
