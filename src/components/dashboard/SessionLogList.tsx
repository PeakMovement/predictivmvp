  ...
  >
    <div className="flex items-center justify-between mb-1.5">
      ...
      <div className="flex items-center gap-1.5">
        <span className="px-2 py-0.5 text-xs rounded-md font-medium bg-blue-500/20 text-blue-400">Training</span>
        {!compareMode && (
...
export const SessionLogList = ({ onCompareRequested }: SessionLogListProps = {}) => {
  ...
  if (isLoading || isFetchingFallback) {
    return (
      <div className="bg-glass rounded-md border border-glass-border p-4 ">
        ...
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-16 bg-glass/30 rounded-md animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <>
      <div className="bg-glass rounded-md border border-glass-border p-4 animate-fade-in">
        ...
        {compareMode && (
          <div className="mb-3 p-2 rounded-md bg-secondary/30 border border-border/50">
