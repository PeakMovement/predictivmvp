...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      console.error("[get-activity-trends] [ERROR] No authorization header");
      return new Response(
    ...
    if (userError || !user) {
      console.error("[get-activity-trends] [ERROR] Invalid token:", userError?.message);
      return new Response(
    ...
    let query = supabase
      ...
      .select("period_date, steps_avg_7d, steps_baseline, steps_delta, calories_avg_7d, calories_baseline, calories_delta, activity_score_avg, trend_direction, updated_at")
      .eq("user_id", user.id)
    ...
    if (error) {
      console.error("[get-activity-trends] [ERROR] Query failed:", error.message);
      return new Response(
    ...
    console.error("[get-activity-trends] [ERROR] Unexpected:", error);
    return new Response(
