...
export const useWearableMetrics = () => {
  ...
  const parseMetrics = useCallback((data: WearableAutoDataRow): ParsedFitbitMetrics => {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const { error } = await supabase.functions.invoke('wearable-fetch-data', { body });
      ...
      toast({
        ...
        description: "Wearable data has been updated successfully",
      });
    } catch (error) {
      console.error('Failed to refresh wearable data:', error);
      toast({
        title: "Refresh Failed",
        description: "Could not refresh wearable data. Please try again.",
        variant: "destructive",
      });
    }
  }, [fetchMetrics]);
  ...
  useEffect(() => {
    ...
    const channel = (supabase as any)
      .channel("wearable_auto_data_changes")
      .on(
        "postgres_changes",
        {
          event: "*",
